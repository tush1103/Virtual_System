...
export const placeOrderController = async (req, res) => {
  try {
    ...
    const existingVoucher=await orderModel.findOne({appliedVoucher: voucherId,userId:req.user._id});
    if(existingVoucher) {
        return res.status(400).send({success:false,message:"Voucher already applied"})
    }
    ...
    if(voucher.type=="discountPerc"){
        ...
        if(!firstOrder){
            ...
            return res.status(400).send({success:false,message:"This is not your first order"})
        }
    }
    else{
        return res.status(400).send({success:false,message:"Invalid voucher"})
    }
